...
def map_file_entry(data: Dict, data_path: str):
    """Map all RecordBase files entry to a Invenio file record entry.

    Args:
        data (Dict): Dict with the RecordBase data.
...
# Record files
#
class RecordFileEntry(UserDict):
    def __init__(self, data=None):
        super(RecordFileEntry, self).__init__(data or {})
...
class RecordFiles(UserList):
    def __init__(self, data=None):
        if not isinstance(data, Sequence):
            raise ValueError("The `data` argument must be a valid sequence type.")

        data = [RecordFileEntry(file_entry) for file_entry in py_.get(data, [])]
        super(RecordFiles, self).__init__(data)


__all__ = ("RecordFiles", "RecordFileEntry", "map_file_entry", "create_file_object")
